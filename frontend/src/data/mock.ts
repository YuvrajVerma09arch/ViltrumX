import type { Severity, Criticality, AutonomyLevel } from '../lib/utils'

/**
 * Demo tenant: PayKraft Technologies — fictional 68-person Bengaluru fintech.
 * One coherent scenario drives every screen (CLAUDE.md §14):
 * impossible-travel login → GitHub PAT abuse → S3 exfil attempt (INC-042).
 */
...
/* ── Live activity feed (Command Deck) ───────────────────────────────── */
...
export type OntNodeType = 'user' | 'service' | 'apikey' | 'device' | 'cloud' | 'repo' | 'saas' | 'data'
...
export const ONT_NODES: OntNode[] = [
...
export type OntEdge = { id: string; source: string; target: string; label: string; attack?: boolean }

export const ONT_EDGES: OntEdge[] = [
...
export const ATTACK_CHAIN: ChainStep[] = [
...
export type Action = {
  ...
  target: string
  level: AutonomyLevel
  status: 'executed' | 'proposed' | 'rolled-back' | 'scheduled'
  blastRadius: number
  time: string
  rollback: string
}
...
/** Full decision trace for the provenance replay view (ACT-2210). */
export const PROVENANCE_TRACE = [
...
export const RISK_TREND = [
  { day: 'Jun 26', org: 22, forecast: null as number | null },
  { day: 'Jun 28', org: 24, forecast: null },
  { day: 'Jun 30', org: 21, forecast: null },
  { day: 'Jul 02', org: 28, forecast: null },
  { day: 'Jul 04', org: 26, forecast: null },
  { day: 'Jul 06', org: 31, forecast: null },
  { day: 'Jul 08', org: 34, forecast: null },
  { day: 'Jul 09', org: 58, forecast: 58 },
  { day: 'Jul 11', org: null as number | null, forecast: 41 },
  { day: 'Jul 13', org: null, forecast: 33 },
]

export const ENTITY_RISK = [
...
export const INV_IDENTITIES = [
  { name: 'arjun.mehta@paykraft.in', kind: 'User · Owner', mfa: true, risk: 18, criticality: 'high' as Criticality },
  { name: 'priya.sharma@paykraft.in', kind: 'User · Admin', mfa: true, risk: 91, criticality: 'high' as Criticality },
  { name: 'rohan.iyer@paykraft.in', kind: 'User', mfa: true, risk: 22, criticality: 'medium' as Criticality },
  { name: 'deepa.nair@paykraft.in', kind: 'User · Finance', mfa: true, risk: 27, criticality: 'high' as Criticality },
  { name: 'ci-deploy-bot', kind: 'Service account', mfa: false, risk: 35, criticality: 'high' as Criticality },
  { name: 'backup-svc', kind: 'Service account', mfa: true, risk: 48, criticality: 'medium' as Criticality },
]

export const INV_CLOUD = [
  { name: 's3://customers-db-backups', kind: 'S3 bucket', posture: 'Encrypted · private · access-logged', risk: 74, criticality: 'crown-jewel' as Criticality },
  { name: 'rds/payments-db', kind: 'RDS Postgres', posture: 'Encrypted · no public endpoint', risk: 31, criticality: 'crown-jewel' as Criticality },
  { name: 'ec2/prod-api', kind: 'EC2 ×6', posture: 'IMDSv2 · patched 4 d ago', risk: 26, criticality: 'high' as Criticality },
  { name: 's3://paykraft-staging-assets', kind: 'S3 bucket', posture: 'Public-read fixed (INC-038)', risk: 12, criticality: 'low' as Criticality },
]

export const INV_REPOS = [
  { name: 'paykraft-core', kind: 'Private repo', posture: 'Branch protection · secret scanning ON', risk: 24, criticality: 'crown-jewel' as Criticality },
  { name: 'paykraft-infra', kind: 'Private repo', posture: 'Secrets in history — purge scheduled', risk: 62, criticality: 'high' as Criticality },
  { name: 'paykraft-web', kind: 'Private repo', posture: 'Branch protection ON', risk: 15, criticality: 'medium' as Criticality },
]

export const INV_SAAS = [
  { name: 'Google Workspace', kind: '68 identities', posture: 'SSO root · admin alerts wired', risk: 20, criticality: 'high' as Criticality },
  { name: 'Razorpay', kind: 'Payment rail', posture: 'Webhook signing · IP allowlist', risk: 22, criticality: 'crown-jewel' as Criticality },
  { name: 'Slack', kind: '64 members', posture: 'App approvals restricted', risk: 9, criticality: 'medium' as Criticality },
]

/* ── Purple team ─────────────────────────────────────────────────────── */

export const PT_SCENARIOS = [
